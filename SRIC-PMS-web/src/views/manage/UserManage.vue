<template>
    <div>
        ...
        <el-drawer v-model="isDrawerOpen" @close="drawerClose" ref="drawer">
            ...
            <el-select v-model="statusEditInput">
                <el-option v-for="(item,i) in levels" :key="i" :label="item.label" :value="item.value">
                    <el-tag :color="item.label" class="userManage-select-color-tag" size="small" />
            ...
            <el-button @click="clickEditStatus()" type="success" style="margin-top: 20px;">
                <el-icon class="userManage-submit-button-icon" size="15">
                    <CircleCheckFilled/>
                </el-icon>
                {{ $t("common.submit") }}
            </el-button>
...
import { getUserMessageList,updateUsernameByUuid,updatePasswordByUuid,updateStatusByUuid } from '@/axios/api/user'
...
const statusEditInput = ref("") //修改权限选择绑定内容
...
const clickEdit = (row :any) => //点击编辑
{
    ...
    passwordInput.value = "*****"
    statusEditInput.value = row.status
}
...
const clickEditStatus = async () => //提交修改权限等级
{
    try
    {
        const params =
        ...
        const resp = await updateStatusByUuid(params)
        if(resp.data.code == 200)
        {
            ElMessage({
                message: t("common.success"),
