<template>
    <el-card>
        ...
        <el-button @click="clickAdd()" type="warning" class="h-video-tag-manage-add-button" plain>
            <el-icon>
                <Plus />
            </el-icon>
    ...
    <el-drawer v-model="isDrawerOpen">
        <template #header>
            <span class="h-video-tag-manage-drawer-title">{{ $t("static.editAndAddTag") }}</span>
        </template>
        <el-form ref="tagFormRef" :model="tagForm" :rules="tagFormRules">
            ...
            <el-form-item>
                <el-button @click="clickAddOrEditTag(tagFormRef)" type="success">
                    <el-icon class="h-video-tag-manage-submit-button-icon" size="15">
                        <CircleCheckFilled/>
                    </el-icon>
                    {{ $t("common.submit") }}
                </el-button>
...
import { getHVideoTagManageDataList,addOrUpdateHVideoTag } from '@/axios/api/hManage';
...
const tagFormRef = ref<FormInstance>() //添加 h video tag 表单的 ref
const tagFormRules = reactive //添加 h video tag 表单的 rule
({
    name:
    [
...
const getHVideoTagData = async () => //获取类别列表
{
    try
    {
        const resp = await getHVideoTagManageDataList({})
...
const doAddOrEditTag = async () =>
{
    try
    {
        ...
        const resp = await addOrUpdateHVideoTag(params)
...
const clickAddOrEditTag = async (formEl: FormInstance | undefined) => //点击添加或修改 h video tag 提交
{
    if (!formEl) return
...
.h-video-tag-manage-add-button
{
    width: 100%;
    margin-top: 20px;
}

.h-video-tag-manage-drawer-title
{
    font-weight: bold;
}

.h-video-tag-manage-submit-button-icon
{
    margin-right: 10px;
}
</style>