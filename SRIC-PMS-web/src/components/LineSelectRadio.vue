<template>
    <el-radio-group v-model="lineNow" @change="lineChange">
        <div class="line-select-radio-group-div">
            ...
            <el-radio value="LOCAL" size="small">{{ $t("common.local") }}</el-radio>
        </div>
    </el-radio-group>
</template>
...
const LOCALurl = inject('LOCALurl')
...
const lineChange = (line:any) => //更改线路
{
    ...
    else if(line == "LOCAL") //本地后端地址
    {
        axios.defaults.baseURL = LOCALurl as string
        localStorage.setItem("line","LOCAL")
        router.push("home")
        ElMessage({
            message: t("static.lineChangeTo") + "LOCAL",
            type: 'success',
        })
    }
    else //参数错误
